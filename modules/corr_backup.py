...
class coin():
    ...
    def data_merge(self, parser):
        ...
        data_list = sorted(Path(data_path).iterdir(), key=os.path.getmtime)
        ##############################################################################################
    ...
    def big_data_merge(self, path='..\\'):
        ...
        for file in url.iterdir():
            arr_toa = np.load(file / 'out_toa.npy')
            arr_xy = np.load(file / 'out_ph.npy')
            arr_ph = np.load(file / 'out_ph.npy')

            npaa_xy.append(arr_xy)
            npaa_toa.append(arr_toa)
            npaa_ph.append(arr_ph)
    ...
    def histogram(self,parameters,offset,delta,binning,split,save=False,cluster=False, show=False):
        ...
        plt.figure()
        plt.title('{}'.format(name)+' \n acquisition: {}s, range = {} ns'.format(parameters['tp3_exposure_time']*parameters['tp3_exposure_time'],delta/1e4)+', binning = {} ns, FWHM = {} ns'.format(2*delta/binning/1e4,fwhm/1e5))
        ...
        np.save(url / 'out_edge_{}.npy'.format(delta),edge)
        return bins[hist.argmax()], hist, edge
    ...
    def coincidences(self,offset,delta,cluster=False):
        ...
        print('Found {} coincidences at {} ns offset'.format(len(coin),offset/1e4))
        np.save(url / 'out_coin.npy',coin.astype(int))

        return coin.astype(int)

    def plot(self,parameters, save_path,delta,offset,thresh,map,scale,save=False,cluster=False,show=False,figsize=(3,3),xlim=(0,255),ylim=(0,255)):
        path = self.path
        url = Path(path) 
        name = self.name
        scaling = 1e-4
        ...
        if cluster == True:
            ax['4'].set(title=r'Clustered: $\tau = {}$ ns; offset = {} ns'.format(delta,offset/1e4),xlim=xlim,ylim=ylim)
            img2 = ax['4'].imshow(bc1-bc3, cmap = map, norm = symlognorm)

        else: 
            ax['3'].set(title=r'$\tau = {}$ ns; offset = {} ns'.format(delta,offset/1e4),xlim=xlim,ylim=ylim)
    ...
    def circular_profile_raw(self, center, radi, parameters, save=False):
            path = self.path
            url = Path(path) 
            name = self.name
            scaling = 1e-4
            ...
            #center = np.unravel_index(np.argmin(bc2-bc1), bc2.shape)
    ...
    def circular_profile_coin(self, center, radi, parameters, save=False):
        path = self.path
        url = Path(path) 
        name = self.name
        scaling = 1e-4
